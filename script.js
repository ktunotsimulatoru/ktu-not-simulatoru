...
// T-Skor Aralıkları (PDF Tablo 1'e göre genişletildi, düşük ortalamalar için ilk aralık kullanılır)
const T_SKOR_ARALIKLARI_ORTALAMAYA_GORE = {
    "0_42.5": { "FF": [-Infinity, 35.99], "FD": [36, 40.99], "DD": [41, 45.99], "DC": [46, 50.99], "CC": [51, 55.99], "CB": [56, 60.99], "BB": [61, 65.99], "BA": [66, 70.99], "AA": [71, Infinity] }, // Kötü ve daha düşükler için
    "42.5_47.5": { "FF": [-Infinity, 33.99], "FD": [34, 38.99], "DD": [39, 43.99], "DC": [44, 48.99], "CC": [49, 53.99], "CB": [54, 58.99], "BB": [59, 63.99], "BA": [64, 68.99], "AA": [69, Infinity] }, // Zayıf
    "47.5_52.5": { "FF": [-Infinity, 31.99], "FD": [32, 36.99], "DD": [37, 41.99], "DC": [42, 46.99], "CC": [47, 51.99], "CB": [52, 56.99], "BB": [57, 61.99], "BA": [62, 66.99], "AA": [67, Infinity] }, // Orta
    "52.5_57.5": { "FF": [-Infinity, 29.99], "FD": [30, 34.99], "DD": [35, 39.99], "DC": [40, 44.99], "CC": [45, 49.99], "CB": [50, 54.99], "BB": [55, 59.99], "BA": [60, 64.99], "AA": [65, Infinity] }, // Ortanın Üstü
    "57.5_62.5": { "FF": [-Infinity, 27.99], "FD": [28, 32.99], "DD": [33, 37.99], "DC": [38, 42.99], "CC": [43, 47.99], "CB": [48, 52.99], "BB": [53, 57.99], "BA": [58, 62.99], "AA": [63, Infinity] }, // İyi
    "62.5_70": { "FF": [-Infinity, 25.99], "FD": [26, 30.99], "DD": [31, 35.99], "DC": [36, 40.99], "CC": [41, 45.99], "CB": [46, 50.99], "BB": [51, 55.99], "BA": [56, 60.99], "AA": [61, Infinity] }, // Çok İyi
    "70_80": { "FF": [-Infinity, 23.99], "FD": [24, 28.99], "DD": [29, 33.99], "DC": [34, 38.99], "CC": [39, 43.99], "CB": [44, 48.99], "BB": [49, 53.99], "BA": [54, 58.99], "AA": [59, Infinity] }  // Mükemmel (<80)
};
...
function getBagilDegerlendirmeNotuTskor(tSkoru, sinifOrtalamasi) { let hedefAralikAnahtari = null; const siraliOrtalamaAraliklari = Object.keys(T_SKOR_ARALIKLARI_ORTALAMAYA_GORE).sort((a, b) => parseFloat(a.split('_')[0]) - parseFloat(b.split('_')[0])); for (const key of siraliOrtalamaAraliklari) { const [minOrtStr, maxOrtStr] = key.split('_'); const minOrt = parseFloat(minOrtStr); const maxOrt = parseFloat(maxOrtStr); if (sinifOrtalamasi > minOrt && sinifOrtalamasi <= maxOrt) { hedefAralikAnahtari = key; break; } } if (!hedefAralikAnahtari) { if (sinifOrtalamasi > 0 && sinifOrtalamasi <= 42.5) { hedefAralikAnahtari = "0_42.5"; } else if (sinifOrtalamasi > 70 && sinifOrtalamasi < 80) { hedefAralikAnahtari = "70_80"; } else if (sinifOrtalamasi >= 80) { console.warn("Ort >= 80 durumu normalde Mutlak sistemdir. T-Skor tablosu 70-80 aralığı kullanılarak tahmin yapılıyor."); hedefAralikAnahtari = "70_80";} else { console.error("Sınıf ortalaması için T-Skor aralığı bulunamadı:", sinifOrtalamasi); return null;} } if (!T_SKOR_ARALIKLARI_ORTALAMAYA_GORE[hedefAralikAnahtari]) { console.error("Tanımlı T-Skor aralığı anahtarı bulunamadı:", hedefAralikAnahtari); return null; } const notlar = T_SKOR_ARALIKLARI_ORTALAMAYA_GORE[hedefAralikAnahtari]; for (const not in notlar) { const [minT, maxT] = notlar[not]; if (tSkoru >= minT && (maxT === Infinity ? true : tSkoru <= maxT)) { return not; } } console.error("T-skor için harf notu bulunamadı. T-Skoru:", tSkoru, "Aralık:", hedefAralikAnahtari); return null; }
    ...
    if (harfNotuFormu) {
        harfNotuFormu.addEventListener('submit', (event) => {
            ...
            // ... Önceki Harf Notu hesaplama kodu (Değişiklik yok) ...
             ...
             if (secilenYontem === 'tek') { const araSinavOrtalamasiInput = document.getElementById('midterm-avg'); const araSinavOrtalamasi = parseFloat(araSinavOrtalamasiInput.value); araSinavOrtalamasiInput.required = true; if (isNaN(araSinavOrtalamasi) || araSinavOrtalamasi < 0 || araSinavOrtalamasi > 100) { hataGoster(harfNotuSonucAlani, "Hata: Geçerli bir ara sınav ortalaması girin (0-100)."); gecerliGiris = false; } else { araSinavKatkisi = araSinavOrtalamasi * 0.50; } document.getElementById('vize-notu-harf').required = false; document.getElementById('vize-agirlik-harf').required = false; document.getElementById('odev-notu-harf').required = false; document.getElementById('odev-agirlik-harf').required = false; }
             else { document.getElementById('midterm-avg').required = false; const vizeNotuInput = document.getElementById('vize-notu-harf'); const vizeAgirlikInput = document.getElementById('vize-agirlik-harf'); const odevNotuInput = document.getElementById('odev-notu-harf'); const odevAgirlikInput = document.getElementById('odev-agirlik-harf'); vizeNotuInput.required = true; vizeAgirlikInput.required = true; odevNotuInput.required = true; odevAgirlikInput.required = true; const vizeNotu = parseFloat(vizeNotuInput.value); const vizeAgirlik = parseFloat(vizeAgirlikInput.value); const odevNotu = parseFloat(odevNotuInput.value); const odevAgirlik = parseFloat(odevAgirlikInput.value); if (isNaN(vizeNotu) || isNaN(vizeAgirlik) || isNaN(odevNotu) || isNaN(odevAgirlik) || vizeNotu < 0 || vizeNotu > 100 || odevNotu < 0 || odevNotu > 100 || vizeAgirlik < 0 || vizeAgirlik > 50 || odevAgirlik < 0 || odevAgirlik > 50) { hataGoster(harfNotuSonucAlani, "Hata: Lütfen detaylı giriş alanlarını (notlar 0-100, ağırlıklar 0-50) doğru şekilde doldurun."); gecerliGiris = false; } else if (Math.abs(vizeAgirlik + odevAgirlik - 50) > 0.01) { hataGoster(harfNotuSonucAlani, "Hata: Detaylı girişteki ağırlıkların toplamı 50 olmalıdır."); gecerliGiris = false; } else { araSinavKatkisi = (vizeNotu * vizeAgirlik / 100) + (odevNotu * odevAgirlik / 100); } }
             if (isNaN(finalNotu) || isNaN(sinifOrtalamasi) || isNaN(sinifStandartSapma)) { hataGoster(harfNotuSonucAlani, "Hata: Lütfen Final Notu, Sınıf Ortalaması ve Standart Sapma alanlarını sayısal olarak doldurun."); gecerliGiris = false; }
             if (finalNotu < 0 || finalNotu > 100 || sinifOrtalamasi < 0 || sinifOrtalamasi > 100 || sinifStandartSapma < 0) { hataGoster(harfNotuSonucAlani, "Hata: Final Notu ve ortalama 0-100, standart sapma 0 veya üzeri olmalıdır."); gecerliGiris = false; }
             if (sinifStandartSapma === 0) { hataGoster(harfNotuSonucAlani, "Hata: Standart sapma 0 olamaz."); gecerliGiris = false; }
             if (!gecerliGiris) return;

             let hamBasariNotu = NaN; let harfNotu = null; let anaMesaj = ""; let tSkoru = null; let hesaplamaDetaylari = "";
             if (finalNotu < MINIMUM_FINAL_NOTU_VARSAYILAN) { harfNotu = "FF"; if (secilenYontem === 'tek') { hamBasariNotu = (parseFloat(document.getElementById('midterm-avg').value) * 0.50) + (finalNotu * 0.50); } else { hamBasariNotu = araSinavKatkisi + (finalNotu * 0.50); } anaMesaj = `Final notunuz (${finalNotu}) minimum (${MINIMUM_FINAL_NOTU_VARSAYILAN}) sınırının altında olduğu için notunuz FF.`;
             } else { if (secilenYontem === 'tek') { hamBasariNotu = araSinavKatkisi + (finalNotu * 0.50); } else { hamBasariNotu = araSinavKatkisi + (finalNotu * 0.50); } if (hamBasariNotu <= 15) { harfNotu = "FF"; anaMesaj = `Hesaplanan Ham Başarı Notu (${hamBasariNotu.toFixed(2)}) 15 veya altında olduğu için notunuz FF.`; } else { tSkoru = ((hamBasariNotu - sinifOrtalamasi) / sinifStandartSapma) * 10 + 50; tSkoru = Math.round(tSkoru * 100) / 100; const bagilNot = getBagilDegerlendirmeNotuTskor(tSkoru, sinifOrtalamasi); if (bagilNot === null) { anaMesaj = `Hata: T-Skoru (${tSkoru}) için Bağıl Değerlendirme harf notu aralığı bulunamadı...`; hataGoster(harfNotuSonucAlani, anaMesaj); return; } const mutlakNotKarsiligi = getMutlakDegerlendirmeNotu(hamBasariNotu); harfNotu = karsilastirHarfNotlari(bagilNot, mutlakNotKarsiligi); hesaplamaDetaylari = `Hesaplanan T-Skoru: <strong>${tSkoru}</strong>.<br>`; hesaplamaDetaylari += `T-skoruna göre Bağıl Değerlendirme notu: ${bagilNot}.<br>`; hesaplamaDetaylari += `Ham Başarı Notunun Mutlak Değerlendirme Sistemindeki karşılığı: ${mutlakNotKarsiligi}.`; } }
             let sonucMesaji = ""; if (anaMesaj) { sonucMesaji += `<p>${anaMesaj}</p><hr>`; } sonucMesaji += `Hesaplanan Ham Başarı Notu: <strong>${hamBasariNotu.toFixed(2)}</strong><br>`; sonucMesaji += `Harf Notu: <strong style="font-size: 1.2em;">${harfNotu}</strong>`; if (hesaplamaDetaylari) { sonucMesaji += `<br><details style="margin-top: 10px; font-size: 0.9em; color: #555;"><summary>Hesaplama Detayları</summary><p style="margin-top: 5px;">${hesaplamaDetaylari}</p></details>`; } if (harfNotu == "DC") { sonucMesaji += "<br><strong>Not:</strong> DC ile geçme durumu dönemlik ağırlıklı genel not ortalamanızın 2.00 ve üzeri olmasına bağlıdır."; } else if (["DD", "FD", "FF"].includes(harfNotu)) { sonucMesaji += `<br><strong>Not:</strong> ${harfNotu} notu başarısız anlamına gelir.`; } harfNotuSonucAlani.innerHTML = sonucMesaji;
        });
    }
    ...
    if (gerekliNotFormu) {
        gerekliNotFormu.addEventListener('submit', (event) => {
            ...
            gerekliNotSonucAlani.innerHTML = "<p>Hesaplanıyor...</p>";
             // ... Önceki Gerekli Final Notu hesaplama kodu (Değişiklik yok) ...
             ...
             let araSinavKatkisi = 0; let gecerliGiris = true;
             if (secilenYontem === 'tek') { const araSinavOrtalamasiInput = document.getElementById('req-midterm-avg'); const araSinavOrtalamasi = parseFloat(araSinavOrtalamasiInput.value); araSinavOrtalamasiInput.required = true; if (isNaN(araSinavOrtalamasi) || araSinavOrtalamasi < 0 || araSinavOrtalamasi > 100) { hataGoster(gerekliNotSonucAlani, "Hata: Geçerli bir ara sınav ortalaması girin (0-100)."); gecerliGiris = false; } else { araSinavKatkisi = araSinavOrtalamasi * 0.50; } document.getElementById('vize-notu-gerekli').required = false; document.getElementById('vize-agirlik-gerekli').required = false; document.getElementById('odev-notu-gerekli').required = false; document.getElementById('odev-agirlik-gerekli').required = false;
             } else { document.getElementById('req-midterm-avg').required = false; const vizeNotuInput = document.getElementById('vize-notu-gerekli'); const vizeAgirlikInput = document.getElementById('vize-agirlik-gerekli'); const odevNotuInput = document.getElementById('odev-notu-gerekli'); const odevAgirlikInput = document.getElementById('odev-agirlik-gerekli'); vizeNotuInput.required = true; vizeAgirlikInput.required = true; odevNotuInput.required = true; odevAgirlikInput.required = true; const vizeNotu = parseFloat(vizeNotuInput.value); const vizeAgirlik = parseFloat(vizeAgirlikInput.value); const odevNotu = parseFloat(odevNotuInput.value); const odevAgirlik = parseFloat(odevAgirlikInput.value); if (isNaN(vizeNotu) || isNaN(vizeAgirlik) || isNaN(odevNotu) || isNaN(odevAgirlik) || vizeNotu < 0 || vizeNotu > 100 || odevNotu < 0 || odevNotu > 100 || vizeAgirlik < 0 || vizeAgirlik > 50 || odevAgirlik < 0 || odevAgirlik > 50) { hataGoster(gerekliNotSonucAlani, "Hata: Lütfen detaylı giriş alanlarını (notlar 0-100, ağırlıklar 0-50) doğru şekilde doldurun."); gecerliGiris = false; } else if (Math.abs(vizeAgirlik + odevAgirlik - 50) > 0.01) { hataGoster(gerekliNotSonucAlani, "Hata: Detaylı girişteki ağırlıkların toplamı 50 olmalıdır."); gecerliGiris = false; } else { araSinavKatkisi = (vizeNotu * vizeAgirlik / 100) + (odevNotu * odevAgirlik / 100); } }
             if (!hedefHarfNotu || isNaN(sinifOrtalamasi) || isNaN(sinifStandartSapma)) { hataGoster(gerekliNotSonucAlani, "Hata: Lütfen Hedef Not, Sınıf Ortalaması ve Standart Sapma alanlarını doğru şekilde doldurun."); gecerliGiris = false; }
             if (sinifOrtalamasi < 0 || sinifOrtalamasi > 100 || sinifStandartSapma < 0) { hataGoster(gerekliNotSonucAlani, "Hata: Sınıf Ortalaması 0-100, standart sapma 0 veya üzeri olmalıdır."); gecerliGiris = false; }
             if (sinifStandartSapma === 0) { hataGoster(gerekliNotSonucAlani, "Hata: Standart sapma 0 olamaz."); gecerliGiris = false; }
             if (!gecerliGiris) return;

             let gerekenFinalNotu = NaN; let anaMesaj = ""; let hesaplamaDetaylari = "";
             const minimumTskor = getHedefNotIcinMinTskor(hedefHarfNotu, sinifOrtalamasi);
             if (minimumTskor === null) { hataGoster(gerekliNotSonucAlani, `Hata: Hedeflenen ${hedefHarfNotu} notu için geçerli bir minimum T-skor bulunamadı...`); return; }
             let hedefHamBasariNotuBagil = ((minimumTskor - 50) / 10) * sinifStandartSapma + sinifOrtalamasi;
             gerekenFinalNotu = 2 * (hedefHamBasariNotuBagil - araSinavKatkisi);
             const gerekenFinalNotuYuvarla = Math.round(gerekenFinalNotu * 100) / 100;
             hesaplamaDetaylari = `Hedeflenen <strong>${hedefHarfNotu}</strong> notu (Bağıl Değerlendirme) için;<br>`;
             hesaplamaDetaylari += `- Gerekli min. T-Skoru: ${minimumTskor.toFixed(2)} (Sınıf Ort: ${sinifOrtalamasi}, Std Sapma: ${sinifStandartSapma})<br>`;
             hesaplamaDetaylari += `- Başarmak için gereken minimum Ham Başarı Notu: <strong>${hedefHamBasariNotuBagil.toFixed(2)}</strong><br>`;
             const mutlakNotKarsiligi = getMutlakDegerlendirmeNotu(hedefHamBasariNotuBagil);
             hesaplamaDetaylari += `<small style='color:#555;'>(Bu Ham Başarı Notu Mutlak Değerlendirme Sisteminde ${mutlakNotKarsiligi} notuna karşılık gelir.)</small>`;
             let sonucMetni = "";
             if (gerekenFinalNotu > 100) { anaMesaj = `Bu Ham Başarı Notuna ulaşmak için gereken final notu (${gerekenFinalNotuYuvarla}) <strong>100'den yüksek</strong>...`; sonucMetni = "İmkansız (>100)"; }
             else if (gerekenFinalNotu < MINIMUM_FINAL_NOTU_VARSAYILAN) { const gosterilecekFinal = Math.max(0, gerekenFinalNotuYuvarla).toFixed(2); anaMesaj = `Bu Ham Başarı Notuna ulaşmak için gereken final notu (${gosterilecekFinal}), minimum final notu (${MINIMUM_FINAL_NOTU_VARSAYILAN}) sınırının altında...`; sonucMetni = `En az ${MINIMUM_FINAL_NOTU_VARSAYILAN} (Hesaplanan: ${gerekenFinalNotuYuvarla})`; }
             else { anaMesaj = `Bu Ham Başarı Notuna ulaşmak için finalden <strong>en az ${gerekenFinalNotuYuvarla}</strong> almanız gerekmektedir.`; sonucMetni = gerekenFinalNotuYuvarla; }
             let sonucMesaji = `Gereken Final Notu: <strong style="font-size: 1.2em;">${sonucMetni}</strong><br><hr>`;
    ...
    if (senaryoFormu) {
        senaryoFormu.addEventListener('submit', (event) => {
            ...
            // Ara sınav katkısını hesapla
             ...
             if (!gecerliGiris) return; // Hata varsa dur
            ...
            tabloHTML += `<thead><tr><th>↓ Std. Sapma \\ Ort. →</th>`;
            senaryoOrtalamalar.forEach(ort => { tabloHTML += `<th>${ort}</th>`; });
            tabloHTML += `</tr></thead><tbody>`;

            senaryoStdSapmalar.forEach(stdSapma => {
                tabloHTML += `<tr><th>${stdSapma}</th>`; // Satır başlığı
                senaryoOrtalamalar.forEach(ortalama => {
                    let gerekenFinalNotu = "Hata"; let cellClass = "impossible";

                    // --- DÜZELTME: Sadece Bağıl Hedefe Göre Hesaplama ---
                    // 1. Hedef T-Skorunu bul
                    const minimumTskor = getHedefNotIcinMinTskor(hedefHarfNotu, ortalama);

                    if (minimumTskor !== null && stdSapma > 0) {
                        // 2. Hedef HBN (Bağıl) hesapla
                         let hedefHamBasariNotuBagil = ((minimumTskor - 50) / 10) * stdSapma + ortalama;

                         // 3. Nihai Hedef HBN SADECE Bağıl hedeften gelir
                         let hedefHamBasariNotuNihai = hedefHamBasariNotuBagil;

                         // 4. Gerekli Finali Hesapla
