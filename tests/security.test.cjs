...
async function upload(body, type = 'application/pdf', claims = {}, length) {
    ...
    const response = await worker.fetch(new Request('https://files.example/upload', { method: 'POST', headers, body, duplex: 'half' }), {
        ...
        NK_ELEMENT_BUCKET: bucketMock(saved)
    });
...
test('ES256/JWKS yükleme yolu geçerli imzayı kabul eder, bozuk imzayı reddeder', async () => {
    ...
    try {
        ...
        const saved = [];
        const env = { SUPABASE_SERVICE_ROLE_KEY: 'test-service', NK_ELEMENT_BUCKET: bucketMock(saved) };
